...
def create_assessment():
    ...
    try:
        ...
        # Translate if needed
        assessment = translate_text(assessment, language)
        
        # Generate document if requested
        if format_type == 'docx':
            doc_buffer = generate_docx(assessment, f"Assessment: {topic}")
            return send_file(doc_buffer, as_attachment=True, 
                           download_name=f"assessment_{topic.replace(' ', '_')}.docx",
                           mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
        elif format_type == 'pdf':
            pdf_buffer = generate_pdf(assessment, f"Assessment: {topic}")
